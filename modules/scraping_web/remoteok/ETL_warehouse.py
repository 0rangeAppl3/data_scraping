  ...
  for page in job_records:
    ...
    for tr in list_tr:
      ...
      if tr.get_attribute('data-id') is not None and tr.get_attribute('data-slug') is None:
        ...
        print(job_wh_record)
        data_to_upsert = [(str(job_wh_record['job_salary']), str(job_wh_record['job_company']), str(job_wh_record['job_title']), str(job_wh_record['job_link']),str(job_wh_record['job_location']),str(job_wh_record['job_skills']),str(job_wh_record['job_description']), str(job_wh_record['website']), job_wh_record['posted_date'])]
        upsert_query = '''
