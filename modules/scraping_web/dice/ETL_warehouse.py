...
def run(data_lake, data_warehouse, driver):
  ...
  job_records = collection.find({'ETL': {'$exists': False}, 'website': 'dice'}, no_cursor_timeout=True).batch_size(10)
  job_list = []
  i = 0
  ...
  for job_record in job_records:
    ...
    try:
      ...
      job_skills_text = []
      job_salary_tag_name = driver.find_element(By.XPATH, '//*[@id="__next"]/div/main/header/div/div/div[2]/ul/ul[2]/li[1]/p')
      job_salary = job_salary_tag_name.text if job_salary_tag_name.get_attribute('data-cy') == 'compensationText' else 'Depends on Experience'
      job_company = driver.find_element(By.XPATH, '//*[@id="__next"]/div/main/header/div/div/div[2]/ul/ul[1]/li[1]').text
      job_title = driver.find_element(By.XPATH, '//*[@id="__next"]/div/main/header/div/div/h1').text
    ...
    data_to_upsert = [(str(job_wh_record['job_salary']), str(job_wh_record['job_skills']), str(job_wh_record['job_company']), str(job_wh_record['job_title']), str(job_wh_record['job_link']), str(job_wh_record['website']), job_wh_record['posted_date'])]
    upsert_query = '''
